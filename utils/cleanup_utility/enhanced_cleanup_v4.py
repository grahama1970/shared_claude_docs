#!/usr/bin/env python3
"""
Enhanced Project Cleanup Utility v4 for Claude Code
Comprehensive validation and cleanup with Git safety measures
Includes automated dependency fixes and daily run capabilities
"""
...
""")
            ...
            self.run_git_command('commit -m "Initial commit - automated Git setup"')
    ...
    def check_git_status(self) -> Dict[str, Any]:
        ...
        if stdout.strip():
            ...
            # Auto-commit if not in dry-run mode
            if not self.dry_run:
                print("  Auto-committing uncommitted changes...")
                self.run_git_command("add -A")
                commit_msg = f"Auto-commit: Pre-cleanup checkpoint - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    def commit_changes(self, message: str) -> bool:
        ...
        # Commit
        commit_message = f"{message}\n\nAutomated cleanup by enhanced_cleanup_v4.py"
...
class EnhancedCleanupUtilityV4:
    """Enhanced cleanup utility with Git safety and comprehensive project validation"""
    
    def __init__(self, config_file='cleanup_config.json', verbose=False, dry_run=False, auto_fix=True):
        ...
        self.report_dir = Path(__file__).parent / 'cleanup_reports'
        ...
        # Project-specific configurations with dependency requirements
        self.project_configs = {
            'sparta': {
                ...
                'required_packages': ['pydantic-settings', 'pydantic']
            },
            'marker': {
                'type': 'tool',
                'requires_cli': True,
            ...
            'youtube_transcripts': {
                ...
                'required_packages': ['youtube-dl', 'yt-dlp', 'whisper']
            },
            'claude_max_proxy': {
                'type': 'proxy',
                'requires_config': True,
                'check_endpoints': True,
                'required_packages': ['httpx', 'fastapi', 'uvicorn', 'pydantic']
            },
            'arxiv-mcp-server': {
                'type': 'mcp',
                'requires_mcp': True,
                'check_mcp_methods': True,
                'required_packages': ['fastapi', 'uvicorn', 'pydantic', 'httpx', 'arxiv']
            },
            'granger_hub': {
                'type': 'hub',
                'requires_tests': True,
                'check_inter_project': True,
                'required_packages': ['pydantic', 'httpx', 'fastapi']
            },
    ...
    def analyze_project_with_git_safety(self, project_path: str) -> Dict[str, Any]:
        ...
        if not git_manager.initialize_git_if_needed():
    ...
    def _fix_file_dependencies(self, project_path: Path, results: Dict[str, Any]):
        ...
        try:
            ...
            # Fix unclosed quotes in dependencies
            fixed_content = re.sub(r'"([^"]+)\[([^\]]+)\](?!")' , r'"\1[\2]"', fixed_content)
            
            if fixed_content != content:
                with open(pyproject_path, 'w') as f:
                    f.write(fixed_content)
                print("  ✅ Fixed syntax errors in pyproject.toml")
            ...
            # Add hatch metadata if needed for direct references
            if file_deps_found and 'tool' in data:
                if 'hatch' not in data['tool']:
                    data['tool']['hatch'] = {}
                if 'metadata' not in data['tool']['hatch']:
                    data['tool']['hatch']['metadata'] = {}
                data['tool']['hatch']['metadata']['allow-direct-references'] = True
                modified = True
            ...
            results['warnings'].append(f"TOML syntax error in pyproject.toml: {e}")
            # Try to fix common syntax errors
            if self.auto_fix and not self.dry_run:
                self._fix_toml_syntax(pyproject_path, results)
        except Exception as e:
            results['warnings'].append(f"Error processing pyproject.toml: {e}")
            if self.verbose:
                traceback.print_exc()
    
    def _fix_toml_syntax(self, pyproject_path: Path, results: Dict[str, Any]):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                # Fix unclosed quotes in array elements
            ...
            if modified:
                ...
                print("  ✅ Fixed TOML syntax errors")
                results['fixes_applied'] = results.get('fixes_applied', [])
                results['fixes_applied'].append("Fixed TOML syntax errors")
        except Exception as e:
            if self.verbose:
                print(f"  Error fixing TOML syntax: {e}")
        ...
        try:
            ...
            for line in lines:
                ...
                if line.strip().startswith('PYTHONPATH='):
                    ...
                    if any(path in value for path in ['./src', '${PWD}/src', 'src', ':/home/graham']):
    ...
    def _fix_missing_dependencies(self, project_path: Path, results: Dict[str, Any], project_config: Dict[str, Any]):
        """Install missing dependencies based on import errors"""
        if not self.auto_fix or self.dry_run:
            return
        
        project_name = project_path.name
        required_packages = project_config.get('required_packages', [])
        ...
        if venv_path.exists():
            ...
            if returncode != 0:
                results['warnings'].append("Failed to create virtual environment")
                return
            pip_cmd = f"{project_path}/.venv/bin/pip"
        ...
        self.run_command(f"{pip_cmd} install pytest pytest-cov pytest-asyncio", cwd=project_path)
        
        results['fixes_applied'] = results.get('fixes_applied', [])
        results['fixes_applied'].append(f"Installed missing dependencies: {', '.join(required_packages)}")
    
    def _fix_module_structure(self, project_path: Path, results: Dict[str, Any], project_config: Dict[str, Any]):
        """Fix module structure issues"""
        if not project_config.get('fix_module_structure') or self.dry_run:
            return
        ...
        if src_dir.exists():
            ...
            if not module_dir.exists():
                print(f"  Creating module directory: src/{module_name}")
                ...
                # Move any Python files from src to module directory
                for py_file in src_dir.glob('*.py'):
                    if py_file.name != '__init__.py':
                        print(f"  Moving {py_file.name} to {module_name}/")
                        py_file.rename(module_dir / py_file.name)
                
                results['fixes_applied'] = results.get('fixes_applied', [])
                results['fixes_applied'].append(f"Fixed module structure for {module_name}")
...
if not success:
    ...
    for submodule in submodules:
        try:
            ...
            print(f"✅ Successfully imported {module_name}.{{submodule}}")
        except ImportError:
            pass  # Optional submodules
        except Exception as e:
            print(f"⚠️  Error importing {module_name}.{{submodule}}: {{e}}")
...
'''
        ...
        try:
            ...
            # Run the test
            stdout, stderr, returncode = self.run_command(
                f'cd "{project_path}" && {python_cmd} _test_imports.py',
                timeout=30
            )
            ...
            if returncode == 0:
                ...
                if self.auto_fix and not self.dry_run:
                    ...
                    self._fix_missing_dependencies(project_path, results, project_config)
                    self._fix_module_structure(project_path, results, project_config)
    ...
    # Include all the validation methods from enhanced_cleanup_v3.py
    ...
    def _validate_tests(self, project_path: Path, results: Dict[str, Any], config: Dict[str, Any]):
        ...
        if not self.dry_run:
            ...
            stdout, stderr, returncode = self.run_command(
                f'{pytest_cmd} -v tests/ --tb=short 2>&1 || true',
                cwd=project_path,
                timeout=120
            )
    ...
    def _add_test_reporter(self, pyproject_path: Path):
        ...
        try:
            ...
            # Also ensure hatch allows direct references
            if 'hatch' not in data['tool']:
                data['tool']['hatch'] = {}
            if 'metadata' not in data['tool']['hatch']:
                data['tool']['hatch']['metadata'] = {}
...
'''
    ...
    def run_scenario_tests(self) -> Dict[str, Any]:
        """Run interaction scenarios as described in docs/big_picture"""
        print("\n🎭 Running Interaction Scenarios...")
        scenario_results = {}
        
        # Look for scenario scripts
        scenario_paths = [
            Path('/home/graham/workspace/shared_claude_docs/docs/test_scenarios'),
            Path('/home/graham/workspace/shared_claude_docs/utils/claude_interactions/scenarios'),
            Path('/home/graham/workspace/shared_claude_docs/utils/claude_interactions/creative_scenarios')
        ]
        ...
        for scenario_dir in scenario_paths:
            ...
            print(f"\n📁 Checking scenarios in: {scenario_dir}")
            
            # Find Python scenario files
            scenario_files = list(scenario_dir.glob('*_scenario.py'))
    ...
    def run(self, parallel: bool = True, run_scenarios: bool = False) -> List[Dict[str, Any]]:
        """Run cleanup on all projects"""
        print("🚀 Enhanced Project Cleanup Utility v4 (with Auto-Fix)")
        ...
        # NOTE: Git operations require sequential processing in live mode
        if not self.dry_run:
            print("⚠️  Live mode requires sequential processing for Git operations")
            parallel = False
        ...
        self.generate_comprehensive_report(all_results, comm_results, scenario_results)
        
        return all_results
    
    def generate_comprehensive_report(self, all_results: List[Dict[str, Any]], 
                                    comm_results: Dict[str, bool],
                                    scenario_results: Dict[str, str] = None):
        """Generate a comprehensive markdown report"""
        report_file = self.report_dir / f"comprehensive_report_{self.timestamp}.md"
        
        with open(report_file, 'w') as f:
            f.write("# 🚀 Enhanced Project Cleanup Report v4 (Auto-Fix Enabled)\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Mode: {'DRY RUN' if self.dry_run else 'LIVE'}\n")
            f.write(f"Auto-Fix: {'ENABLED' if self.auto_fix else 'DISABLED'}\n\n")
            ...
            if scenario_results:
                ...
                for scenario, status in scenario_results.items():
                    ...
                    f.write(f"| {scenario} | {icon} {status} |\n")
                f.write("\n")
            ...
            # Detailed Results (abbreviated for space)
            f.write("\n## 📝 Key Issues Summary\n\n")
            ...
            f.write("### Daily Cleanup Routine\n")
            f.write("1. Run this cleanup script every morning\n")
            f.write("2. Review and merge any pending cleanup branches\n")
            f.write("3. Run scenario tests to ensure inter-project compatibility\n")
            f.write("4. Address any new issues identified\n\n")
            
            f.write("### Automation Setup\n")
            f.write("Add to your crontab for daily runs:\n")
            f.write("```bash\n")
            f.write("0 6 * * * cd /home/graham/workspace/shared_claude_docs/utils/cleanup_utility && ")
            f.write("./run_enhanced_cleanup_v4.sh --localhost --live --scenarios\n")
            f.write("```\n\n")
            
            f.write("### Next Steps\n")
            f.write("1. Address all critical issues (failed tests)\n")
            f.write("2. Review and merge cleanup branches created by this tool\n")
            f.write("3. Fix any remaining import issues\n")
            f.write("4. Add missing documentation (README.md, CLAUDE.md)\n")
            f.write("5. Ensure all projects have proper test coverage\n")
            f.write("6. Set up automated daily runs\n")
        ...
        with open(summary_file, 'w') as f:
            f.write(f"Enhanced Cleanup Summary v4 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"{'='*70}\n")
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced Project Cleanup Utility v4')
    ...
    parser.add_argument('--no-auto-fix', action='store_true', help='Disable automatic dependency fixes')
    parser.add_argument('--scenarios', action='store_true', help='Run interaction scenarios after cleanup')
    ...
    utility = EnhancedCleanupUtilityV4(
    ...
    if skipped > 0:
        ...
        print("These have been auto-committed and will be processed next run.")
