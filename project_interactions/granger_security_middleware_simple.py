...
class GrangerSecurity:
    ...
    def remove_stack_traces(self, error_message: str) -> str:
        ...
        # Also handle patterns like api_key=value
        cleaned = re.sub(r'(api_key|apikey|password|token|secret|API_KEY)\s*=\s*[\w\-]+', '[redacted]=[redacted]', cleaned, flags=re.IGNORECASE)
        
        # Handle sk- pattern for API keys
        cleaned = re.sub(r'sk-[\w\-]+', '[redacted]', cleaned)
