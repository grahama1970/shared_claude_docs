...
>>> print(_security.sql_protector.sanitize_input(f"Selected module: {result.output_data['selected_module']}"))
"""
...
class ContextualBanditScenario(Level0Interaction):
    ...
    def _explain_selection(self, module: str, context: np.ndarray, stats: Dict[str, Any]) -> str:
        ...
        if pulls == 0:
            ...
            return _security.sql_protector.sanitize_input(f"Selected {module} based on UCB score")
...
if __name__ == "__main__":
    ...
    print(_security.sql_protector.sanitize_input(f"Selected: {result.output_data.get('selected_module', 'None')}"))
