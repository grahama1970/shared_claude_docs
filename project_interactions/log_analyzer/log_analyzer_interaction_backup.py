...
class DistributedLogAnalyzer:
    ...
    def analyze_logs(self, log_files: List[str], streaming: bool = False) -> Dict[str, Any]:
        ...
        with Pool(self.worker_count) as pool:
            file_chunks = self._distribute_files(log_files)
            results = pool.map(self._process_file_batch, file_chunks)
            
            for file_logs in results:
    ...
    def _distribute_files(self, files: List[str]) -> List[List[str]]:
        """Distribute files among workers"""
        chunks = [[] for _ in range(self.worker_count)]
        
        for i, file_path in enumerate(files):
            chunks[i % self.worker_count].append(file_path)
        
        return chunks
    
    def _process_file_batch(self, files: List[str]) -> List[LogEntry]:
...
if __name__ == "__main__":
    ...
    # Write sample logs
    log_dir = Path('/home/graham/workspace/shared_claude_docs/project_interactions/log-analyzer/logs/samples')
